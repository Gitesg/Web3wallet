...
class SparklineIn7D {
  ...
  factory SparklineIn7D.fromJson(Map<String, dynamic> json) {
    final priceList = json["price"];
    if (priceList != null && priceList is List<dynamic>) {
      return SparklineIn7D(
        price: List<double>.from(priceList.map((x) => x?.toDouble() ?? 0.0)),
      );
    } else {
      // Return a default value or throw an exception, depending on your use case.
      // Here, I'm returning an empty list.
      return SparklineIn7D(
        price: [],
      );
    }
